...
app.controller('DemoAppController', function($http, $location, $uibModal) {
    ...
    demoApp.getSecurityAgreements = () => $http.get(apiBaseURL + "securityagreements")
        .then((response) => demoApp.securityagreements = Object.keys(response.data)
            .map((key) => response.data[key].state.data)
            .reverse());

    demoApp.getMySecurityAgreements = () => $http.get(apiBaseURL + "my-securityagreements")
        .then((response) => demoApp.mysecurityagreements = Object.keys(response.data)
            .map((key) => response.data[key].state.data)
            .reverse());

    demoApp.getSecurityAgreements();
    demoApp.getMySecurityAgreements();
...
app.controller('ModalInstanceCtrl', function ($http, $location, $uibModalInstance, $uibModal, demoApp, apiBaseURL, peers) {
    ...
    // Validate and create SecurityAgreement.
    modalInstance.create = () => {
        if (invalidFormInput()) {
            ...
            const createSecurityAgreementEndpoint = `${apiBaseURL}create-securityagreement?partyName=${modalInstance.form.counterparty}&securityAgreementValue=${modalInstance.form.value}`;

            // Create PO and handle success / fail responses.
            $http.put(createSecurityAgreementEndpoint).then(
                (result) => {
    ...
    // Close create SecurityAgreement modal dialogue.
    modalInstance.cancel = () => $uibModalInstance.dismiss();

    // Validate the SecurityAgreement.
